...
module Domain.Project (Project(..), ProjectId, showElements, showElementsName, createEmptyProject, flatten, extractScene, getFollowingScenes) where
...
import           Domain.Scene           (Scene (..), sceneName, scenePosition)
...
getFollowingScenes :: Int -> Project -> [Scene]
getFollowingScenes scnPosition = filter ((> scnPosition) . scenePosition). sortOn scenePosition . projectScenes
